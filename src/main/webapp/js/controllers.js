angular.module('authApp.controllers', []).controller('UserListController', function($scope, $state, popupService, $window, User) {
    ...
    $scope.login = function() {
        ...
        return $http({
            method: 'POST',
            url: 'http://localhost:8080/auth-playground/resources/auth/login/',
            data: credentials,
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
        }).then(function(res) {
            $http.defaults.headers.common['auth_token'] = res.data.auth_token;
        });
    }
});
